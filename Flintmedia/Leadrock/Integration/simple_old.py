...
class IntegrationLeadrock(unittest.TestCase):
    ...
    #Url parsing
        simple = byxpath('//*[@id="subscribe-list"]/table/tbody/tr[3]/td[2]').text
        if "Обычная старая" in simple:
            simple_url = byxpath('//*[@id="subscribe-list"]/table/tbody/tr[3]/td[5]/span').text
    ...
    #Simple landing + checking lead availability
        print('Simple landing test')
        driver.get(simple_url)
        current = driver.current_url
        trkid = re.search(r'\=([\d]*)', current)
    ...
    #Simple checking
        ...
        if "confirm" in str(confirm):
            ...
            trkid_check = byxpath('//*[@id="lead-list"]/table/tbody/tr[1]/td[2]/a').text
            if str(trkid.group(1)) in str(trkid_check):
                print('Simple lead integration OK!\n')
            else:
                print('NO LEAD!!')
                driver.quit()
        else:
